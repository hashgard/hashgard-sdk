...
import (
	"fmt"
	"github.com/cosmos/cosmos-sdk/x/scene"
	"io"
	"os"
	"reflect"
	"runtime/debug"
...
type BaseApp struct {
	...
	// HashGard
	sceneKeeper    *scene.Keeper    // scene keeper for scene record
...
func (app *BaseApp) runMsgs(ctx sdk.Context, msgs []sdk.Msg, mode runTxMode) (result sdk.Result) {
	...
	for i, msg := range msgs {
		...
		// HashGard
		if app.GetSceneKeeper() != nil {
			if err := app.GetSceneKeeper().AddTxScene(ctx, msg.Route(), msg.GetSigners()); err != nil {
				return err.Result()
			}
		}
...
//HashGard
// SceneKeeper returns the scene keeper for scene record
func (app *BaseApp) SetSceneKeeper(keeper *scene.Keeper) {
	app.sceneKeeper = keeper
}

//HashGard
// SceneKeeper returns the scene keeper for scene record
func (app *BaseApp) GetSceneKeeper() *scene.Keeper {
	return app.sceneKeeper
}
