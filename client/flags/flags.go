...
func PostCommands(cmds ...*cobra.Command) []*cobra.Command {
	for _, c := range cmds {
		...
		// HashGard
		c.Flags().String(FlagFees, "", "Fees to pay along with transaction; eg: 50000ugard")
		c.Flags().String(FlagGasPrices, "", "Gas prices to determine the transaction fee (e.g. 25000ugard)")
		c.Flags().String(FlagNode, "tcp://localhost:26657", "<host>:<port> to tendermint rpc interface for this chain")
		c.Flags().Bool(FlagUseLedger, false, "Use a connected Ledger device")
		c.Flags().Float64(FlagGasAdjustment, DefaultGasAdjustment, "adjustment factor to be multiplied against the estimate returned by the tx simulation; if the gas limit is set manually this flag is ignored ")
		// HashGard
		c.Flags().StringP(FlagBroadcastMode, "b", BroadcastBlock, "Transaction broadcasting mode (sync|async|block)")
...
func RegisterRestServerFlags(cmd *cobra.Command) *cobra.Command {
	...
	// HashGard
	cmd.Flags().String(FlagSignAddr, "", "Transaction proxy signature address list, used for intranet service, please do not set when LCD is used for internet service")
	return cmd
}
