...
The Cosmos SDK Light Client (Hashgard-lite) is split into two separate components. The first component is generic for
...
API as any Tendermint Core node. The second component is specific for the Cosmos Hub (`hashgardd`). It works as a query
...
All applications are expected to run only against Hashgard-lite. Hashgard-lite is the only piece of software
...
|| Full Node | Hashgard-lite | Description|
|-| ------------- | ----- | -------------- |
| Execute and verify transactions|Yes|No|A full node will execute and verify all transactions while Hashgard-lite won't.|
| Verify and save blocks|Yes|No|A full node will verify and save all blocks while Hashgard-lite won't.|
...
| Storage resources|High|Low|A full node will save all blocks and ABCI states. Hashgard-lite just saves validator sets and some checkpoints.|
| Power consumption|High|Low|Full nodes must be deployed on machines which have high performance and will be running all the time. Hashgard-lite can be deployed on the same machines as users' applications, or on independent machines but with lower performance. Light clients can be shut down anytime when necessary. Hashgard-lite consumes very little power, so even mobile devices can meet the power requirements.|
| Provide APIs|All cosmos APIs|Modular APIs|A full node supports all Cosmos APIs. Hashgard-lite provides modular APIs according to users' configuration.|
...
According to the above table, Hashgard-lite can meet many users' functionality and security requirements, but require little bandwidth, computing, storage, and power.

## Achieving Security

### Trusted Validator Set

The base design philosophy of Hashgard-lite follows two rules:
...
validator set comes from a genesis file. During runtime, if Hashgard-lite detects a different validator set,
