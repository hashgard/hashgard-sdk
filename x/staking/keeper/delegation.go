...
// CompleteRedelegationWithAmount completes the redelegations of all mature entries
// in the retrieved redelegation object and returns the total redelegation (initial)
// balance or an error upon failure.
	...
	for i := 0; i < len(red.Entries); i++ {
		...
		if entry.IsMature(ctxTime) {
			...
			if !entry.InitialBalance.IsZero() {
				balances = balances.Add(sdk.NewCoins(sdk.NewCoin(bondDenom, entry.InitialBalance)))
			}
		}
	}
