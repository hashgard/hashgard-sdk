...
func NewQuerier(k Keeper) sdk.Querier {
	return func(ctx sdk.Context, path []string, req abci.RequestQuery) (res []byte, err sdk.Error) {
		switch path[0] {
		// HashGard
		case types.QueryBlocks:
			return queryValidatorBlocks(ctx, path[1], path[2], path[3], req, k)
...
// HashGard
func queryValidatorBlocks(ctx sdk.Context, valAddr string, startIdStr string, limitStr string, req abci.RequestQuery, k Keeper) ([]byte, sdk.Error) {
	...
	if limit > 100 {
		return nil, sdk.ErrInternal("limit max 100")
	}
	list := k.GetStakingBlocks(ctx, addr, startID, limit)
