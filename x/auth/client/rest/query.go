...
// HashGard
func QueryAccountHoldersRequestHandlerFn(storeName string, cliCtx context.CLIContext) http.HandlerFunc {
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if len(strNumLimit) > 0 {
			...
			if limit > 100 {
				rest.WriteErrorResponse(w, http.StatusInternalServerError, fmt.Errorf("limit must less than 100").Error())
				return
			}
		}
		vars := mux.Vars(r)
		res, _, err := cliCtx.QueryWithData(fmt.Sprintf("custom/%s/%s/%s/%s/%s", storeName, types.QueryAccountHolders,
			vars[denom], r.URL.Query().Get("start_id"), strNumLimit), nil)
